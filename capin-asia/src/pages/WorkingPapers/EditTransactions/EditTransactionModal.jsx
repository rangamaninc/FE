...
export default function EditTransactionModal({
  ...
  const fetchSubTransactions = useCallback(async () => {
    ...
    const res = await getSubTransactionsByTransactionId(
      selectedClientId,
      selectedGLCode,
      selectedTransactionId
    );

    const subTxns = res.subTransactions;
    const existingGLRows = [];
    const existingGLRowsData = {};
    subTxns.map((subTxn, index) => {
      existingGLRowsData[index] = {
        masterCode: subTxn.glcode,
        amount: parseFloat(subTxn.amount),
        description: subTxn.description,
      };
      existingGLRows.push(index + 1);
    });
    setGLRows(existingGLRows);
    setGLRowsData(existingGLRowsData);
  }, [
    selectedClientId,
    selectedGLCode,
    selectedTransactionId,
  ...
  }, [fetchSubTransactions, showModal]);
  ...
  const handleSaveTransaction = async () => {
    ...
    const res = await updateSubTransactions(
      selectedClientId,
      selectedGLCode,
      selectedTransactionId,
      reqData
            ...
            <Grid container spacing={2} rowSpacing={3}>
                  ...
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DemoContainer components={["DatePicker"]}>
                      <DatePicker
                        ...
                        defaultValue={dayjs(txnData?.transactionDate)}
                      />
                    </DemoContainer>
              ...
              <Grid item xs={4}>
                <Select
                  ...
                  defaultValue={
                    txnData?.transactionType === "payment"
                      ...
                      >
                        <Grid item xs={6}>
                          <Select
                            ...
                            classNamePrefix="select"
                            defaultValue={
                              glRowsData[index]?.masterCode
                                ? {
                                    value: glRowsData[index]?.masterCode,
                                    label: `${
                                      glRowsData[index]?.masterCode
                                    } - ${
                                      clientGLCodesMap[
                        ...
                        <Grid item xs={2}>
                          <TextField
                            ...
                            value={parseFloat(glRowsData[index]?.amount)}
                            InputLabelProps={{ shrink: true }}
                          />
                        </Grid>
                        <Grid item xs={4}>
                          <TextField
                            ...
                            value={glRowsData[index]?.description}
                            InputLabelProps={{ shrink: true }}
                          />
                        </Grid>
                      </Grid>
...
EditTransactionModal.propTypes = {
  ...
  selectedGLCode: PropTypes.string.isRequired,
  selectedTransactionId: PropTypes.number.isRequired,
